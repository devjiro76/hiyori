...
export interface ApplyEmotionResult {
  newEmotion: string
 ...
 *
 * Returns the new emotion label and an optional motion to play.
 * Caller is responsible for tracking prevEmotion across calls.
 */
export function applyEmotionToLive2D(
  ...
  let motion: { group: string; index: number } | undefined
  if (prevEmotion && prevEmotion !== primary) {
    motion = EMOTION_TO_MOTION[primary]
    if (motion) {
      controller.playMotion(motion.group, motion.index)
    }
  }

  return { newEmotion: primary, motion }
}
